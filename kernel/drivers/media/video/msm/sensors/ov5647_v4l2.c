...
/* -------------------------------------------------------------------------- */
/*  Author :  pengdong.su                                                     */
/*  Email  :  pengdong.su@tcl-mobile.com                                      */
/*  Role   :                                                                  */
/*  Reference documents :                                                     */
/* -------------------------------------------------------------------------- */
/*  Comments :                                                                */
...
/* ----------|----------------------|----------------------|----------------- */
/* 12/07/2012|pengdong.su           |PR-365263             |a few ripples on  */
/*           |                      |                      |preview screen    */
/* ----------|----------------------|----------------------|----------------- */
/* 12/11/2012|XuNan                 |347828                |Camera time perfo */
/*           |                      |                      |rmance            */
/* ----------|----------------------|----------------------|----------------- */
/* 01/24/2013|Qigaoyuan             |384135                |Screen flash      */
/* ----------|----------------------|----------------------|----------------- */
/******************************************************************************/
...
//[PLATFORM]-Mod-BEGIN by TCTNB.HJ 2012/12/10, for scribe5-ov5647's OTP, bug371680
extern int ov5647_v4l2_update_otp(struct msm_sensor_ctrl_t *ov5647_s_ctrl, u8 golden_rg, u8 golden_bg);
//[PLATFORM]-Mod-END by TCTNB.HJ 2012/12/10, for scribe5-ov5647's OTP, bug371680
static struct msm_sensor_ctrl_t ov5647_s_ctrl;
//[BUGFIX]-Add-BEGINÂ byÂ TCTNB.Qigaoyuan,1/23/2013,384135,screen flash
...
// Modified the two parameters in PREVIEW, in order to
// make the solution of AFD ripple issus is compatible
// to the CTS test case testCancelAutoFocus.
// Comment by TCTNB(Alan Wang)  @2012-12-26
static struct msm_sensor_output_info_t ov5647_dimensions[] = {
	...
	{ /* For PREVIEW */
		...
		.vt_pixel_clk = 55200000,     //55969920,   Mod by TCTNB(Alan Wang)//Mod by XuNan 55300000 -> 55200000 for testPreviewFpsRange,testCancelAutofocus,testCameraToSurfaceTextureMetadata CTS issue
		.op_pixel_clk = 130000000,    //15940800,   Mod by TCTNB(Alan Wang)
		.binning_factor = 0x0,
	},
	{ /* For 60fps */
		.x_output = 0x280,  /*640*/
		...
		uint16_t gain, uint32_t line)
{
	...
	CDBG(KERN_ERR "snapshot exposure seting 0x%x, 0x%x, %d"
        ...
        if(line > 1169)
            exposure_delay = 1;
        //[BUGFIX]-Add-ENDÂ byÂ TCTNB.Qigaoyuan
...
//[BUGFIX]-Add by TCTBN.XuNan,12/11/2012,PR347828,time performance
static int is_first_preivew = 1;
			...
			int update_type, int res)
{
	...
	//[BUGFIX]-Mod-BEGIN by TCTBN.XuNan,12/11/2012,PR347828,time performance
	...
	if (update_type == MSM_SENSOR_REG_INIT) {
		...
		msleep(50);
//[PLATFORM]-Mod-BEGIN by TCTNB.HJ 2012/12/10, for scribe5-ov5647's OTP, bug371680
		otp_ret = ov5647_v4l2_update_otp(&ov5647_s_ctrl, 0x59, 0x5c);
		if (otp_ret == 0) {
			printk("\n----------- scribe5's ov5647's OTP update SUCCEED!!!-----------\n");
        } else if (otp_ret == 1){
			printk("\n----------- scribe5's ov5647's have no OTP!!!-----------\n");
        } else {
			printk("\n----------- scribe5's ov5647's OTP update FAILED!!!-----------\n");
        }
//[PLATFORM]-Mod-END by TCTNB.HJ 2012/12/10, for scribe5-ov5647's OTP, bug371680
	}
	return rc;
