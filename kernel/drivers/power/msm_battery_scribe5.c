 ...
 * -----------------------------------------------------------------------------
 * 12/11/08       YuBin       Create this file
 * 12/11/14       YuBin       CP to control battery voltage
 * -----------------------------------------------------------------------------
 */
...
struct msm_battery_info {
        ...
        s8 chg_evt_count;//to filter "charger event callback update voltage display"

        bool is_45_stop_charging;//to control high battery_temp charging
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/11/30, Add control for USB instert/remove*/
#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
extern void set_tp_status_for_chg(int enable);
static u32 current_chg_status = 0;
u32 get_chg_status_for_tp(void)
{
        return current_chg_status;
}
#endif
...
{
        u32        charger_status = CHARGER_STATUS_INVALID;
        u32        charger_type;
        u32        battery_status;
        u32        battery_level;
        u32     battery_voltage;
        ...
        bool is_awake = !msm_batt_info.is_suspended;

        mutex_lock(&msm_batt_info.update_mutex);

/* get charging status from ATC */
...
/* charger event handle */
        if (msm_batt_info.charger_type != charger_type) {
                if (charger_type == CHARGER_TYPE_USB_WALL) {
                        DBG_LIMIT("BATT: AC charger plugged in\n");
                        if(!wake_lock_active(&msm_batt_info.charger_wake_lock))
                            wake_lock(&msm_batt_info.charger_wake_lock);
                        msm_batt_info.current_chg_source = AC_CHG;
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/11/30, Add control for USB instert/remove*/
#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
						current_chg_status = AC_CHG;
...
/*[PLATFORM]-Add-END by TCTNB.WQF*/
                        ...
                        msm_batt_info.is_45_stop_charging = false;
                        ...
                        if(wake_lock_active(&msm_batt_info.charger_wake_lock))
                                wake_unlock(&msm_batt_info.charger_wake_lock);


/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/11/30, Add control for USB instert/remove*/
#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
						current_chg_status = 0;
...
/* control NCP1851 start charging */
        if (msm_batt_info.charger_status != charger_status) {
                if(charger_type == CHARGER_TYPE_USB_WALL)
                        start_usb_wall_charging();
                else if((charger_type == CHARGER_TYPE_USB_PC)||(charger_type == CHARGER_TYPE_USB_CARKIT))
                        start_usb_pc_charging();
...
/* battery_temp control when charging */
        if(charger_status == CHARGER_STATUS_GOOD)
        {
                if((false == msm_batt_info.is_45_stop_charging)&&((battery_temp >= 50) || (battery_temp <= 0)))
                {
                        msm_batt_info.is_45_stop_charging = true;
                ...
                else if((true == msm_batt_info.is_45_stop_charging)&&((battery_temp <= 48)&&(battery_temp >= 2)))
                {
                        msm_batt_info.is_45_stop_charging = false;
        ...
        if (msm_batt_info.battery_temp != battery_temp){
                ...
                if (msm_batt_info.battery_voltage != battery_voltage) 
                {
                                ...
                                msm_batt_info.batt_capacity = msm_calculate_batt_capacity(battery_voltage);
        ...
        mutex_unlock(&msm_batt_info.update_mutex);

        if(is_awake||(charger_status == CHARGER_STATUS_GOOD)) {
                queue_delayed_work(msm_batt_info.battery_queue,
                                   &msm_batt_info.battery_work,
                                   10*HZ);
        }
...
*/
        mutex_lock(&msm_batt_info.suspend_lock);
        if(msm_batt_info.is_suspended) {
                mutex_unlock(&msm_batt_info.suspend_lock);
                pr_err("BATT: CAUTION: %s, is already suspended\n", __func__);
                return;
        }
        msm_batt_info.is_suspended = true;
        mutex_unlock(&msm_batt_info.suspend_lock);
        flush_workqueue(msm_batt_info.battery_queue);
                            ...
                            void *buffer, int in_size)
{
        ...
        DBG_LIMIT("\n  -----msm_batt_cb_func--------is_suspend:%d \n",msm_batt_info.is_suspended);
        ...
        {
                wake_lock_timeout(&msm_batt_info.charger_cb_wake_lock,
                                  10 * HZ);
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
...
#ifndef CONFIG_BATTERY_MSM_FAKE
        rc = msm_batt_register(msm_batt_info.voltage_fail_safe, BATTERY_ALL_ACTIVITY,
                               BATTERY_CB_ID_ALL_ACTIV, BATTERY_ALL_ACTIVITY);
        if (rc < 0) {
                dev_err(&pdev->dev,
        ...
        msm_batt_info.is_suspended = false;
        msm_batt_info.is_resumed = false;
        mutex_init(&msm_batt_info.suspend_lock);
        register_early_suspend(&msm_batt_info.early_suspend);
#endif

        msm_batt_info.battery_queue = create_singlethread_workqueue(
                        "battery_queue");
        if (!msm_batt_info.battery_queue) {
                printk(KERN_ERR "[msm_battery.c] error: create_singlethread_workqueue failed!");
                msm_batt_cleanup();
                return -ENOMEM;
        }
        INIT_DELAYED_WORK(&msm_batt_info.battery_work, msm_battery_worker);
        mutex_init(&msm_batt_info.update_mutex);

        wake_lock_init(&msm_batt_info.charger_cb_wake_lock, WAKE_LOCK_SUSPEND,
                       "msm_charger_cb");

        wake_lock_init(&msm_batt_info.charger_wake_lock, WAKE_LOCK_SUSPEND,
                       "msm_charger");
        ...
        //msm_batt_update_psy_status();
        DBG_LIMIT("\n fail_safe=%d;  min=%d; max=%d \n",msm_batt_info.voltage_fail_safe,msm_batt_info.voltage_min_design,msm_batt_info.voltage_max_design);
#else
...
static int __devexit msm_batt_remove(struct platform_device *pdev)
{
        ...
        wake_lock_destroy(&msm_batt_info.charger_cb_wake_lock);
        wake_lock_destroy(&msm_batt_info.charger_wake_lock);
