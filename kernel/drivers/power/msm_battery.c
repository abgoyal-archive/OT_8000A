 ...
 * -----------------------------------------------------------------------------
 * 12/09/28       YuBin       Add bad temp control, standby control, state
 *                            correct for charging
 * 12/10/08       YuBin       Add temp display
 * 12/10/28       YuBin       Add temp < 0 stop charging
 * -----------------------------------------------------------------------------
 */
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/9/11, Add this .h file to use ncp1851 api for scribe5*/
#ifdef CONFIG_CHARGER_IC_NCP1851
#include "./ncp1851.h"
#endif
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/9/28, scribe 5 battery is 4350mV*/
#ifdef CONFIG_CHARGER_IC_NCP1851
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/9/28, Add this one for stop charging when temp bad*/
#ifdef CONFIG_CHARGER_IC_NCP1851
       u32 is_45_stop_charging;
#endif
/*[PLATFORM]-Add-END by TCTNB.YuBin*/
};
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/10/08, Add temp display for scribe5*/
#ifdef CONFIG_CHARGER_IC_NCP1851
...
static void msm_batt_update_psy_status(void)
{
	...
	u32	battery_level;
	u32     battery_voltage;
	s32	battery_temp;/*[PLATFORM]-Mod by TCTNB.YuBin, 2012/10/28, Battery temp need to be signed int*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/9/28, It is no need for scribe 5*/
#ifndef CONFIG_CHARGER_IC_NCP1851
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/9/28, correct state for ncp1851*/
#ifdef CONFIG_CHARGER_IC_NCP1851
       if(battery_status != CHARGER_STATUS_INVALID)
       {
	       if(battery_status != BATTERY_STATUS_GOOD)
	       {
	               stop_charging();
	       }

	       if((battery_temp > 45) && (battery_temp < 0))
	       {
	               msm_batt_info.is_45_stop_charging = 1;
	               stop_charging();
	       }

	       if((1 == msm_batt_info.is_45_stop_charging)&&((battery_temp <= 43)||(battery_temp >= 0)))
	       {
	               msm_batt_info.is_45_stop_charging = 0;
	               restart_charging();
	       }

	       if(is_battery_full())
	              battery_voltage = 4350;
	       else
	       {
	              if(battery_voltage >= 4350)
	                      battery_voltage = 4349;
	       }

	       if(msm_batt_info.is_45_stop_charging == 0)
	       {
	              if(charger_type == CHARGER_TYPE_USB_WALL)
       ...
       if(battery_voltage < 3200)
              battery_level = BATTERY_LEVEL_DEAD;
       else if(battery_voltage < 3400)
              battery_level = BATTERY_LEVEL_WEAK;
       else if(battery_voltage < 4350)
...
/*[PLATFORM]-Add-BEGIN by TCTNB.YuBin, 2012/9/28, for standby charging */
#ifdef CONFIG_CHARGER_IC_NCP1851
				VBATT_CHG_EVENTS,
