...
*           08/21/2012 - Transplanted from ics project by TCTNB.XQJ
*           08/21/2012 - Mod TCTNB.XQJ,Change calibration mode and 
                        -change some parameters for Tmd27723
                                                                *
********************************************************************************
...
#endif
static void taos_prox_calibrate(void);
// first device number
...
static struct file_operations taos_fops = { .owner = THIS_MODULE, .open = taos_open, .release =
	    taos_release, .read = taos_read, .write = taos_write, .llseek = taos_llseek, .unlocked_ioctl = taos_ioctl, 
};
...
//static u8 prox_pulse_cnt_param = 0x08;//[BUG]-DEL by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
static u8 prox_pulse_cnt_param = 0x05; //[BUG]-ADD by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test

//static u8 prox_gain_param = 0x60;//[BUG]-DEL by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
static u8 prox_gain_param = 0x64;//[BUG]-ADD by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
...
//=========================================
static int lightsensor_setup(void)
{
	int ret;
...
#if 0
    /*Need add psensor interrupt gpio setup,now interrupt not been used*/
...
static int taos_probe(struct i2c_client *clientp, const struct i2c_device_id *idp)
{
    ...
    taos_prox_calibrate();/*[FEATURE]-ADD by TCTNB.XQJ,2012/08/20,FR-316014, FR-316022,FR-316019,placed here for proximity calibrate*/   
	return (ret);
}
...
static int taos_prox_on(void)
{
    ...
    /*set offset 630-100=530,530/6=106,0x6A bit7 0 ,0x6A*/
	if ((ret =
	      (i2c_smbus_write_byte_data
           (taos_datap->client, (TAOS_TRITON_CMD_REG | 0x0), 0))) < 0) {
        printk(KERN_ERR "TAOS: i2c_smbus_write_byte_data failed in ioctl prox_on\n");
        return (ret);
    }
...
static long taos_ioctl( struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case TAOS_IOCTL_PROX_CALIBRATE:
	    /*[FEATURE]-DEL-BEGIN by TCTNB.XQJ,2012/08/20,FR-316014, FR-316022,FR-316019, change from running calibartion before
		using proximity sensor,to only calibarte once on mobile power on,and added 
		sample data.new calibrate code has been removed to function  taos_prox_calibrate() 
		*/
           /*FEATURE]-END by TCLNB.XQJ*/
		break;
...
/*[FEATURE]-ADD by TCTNB.XQJ,2012/08/20,FR-316014, FR-316022,FR-316019,
            change from running calibartion before
            using proximity sensor,to only calibarte once on mobile power on,and added 
	        sample data.new calibrate code be remove to function  taos_prox_calibrate() 
*/
static void taos_prox_calibrate(void)
{
    int ret,i;
    ...
    ret = taos_prox_on();
    if (ret < 0) {
        printk("XXXXXXXXXXXXXXXXX TAOS_PROX_CALIBRATE_ON_ ERROR!!! XXXXXXXXXXXXX\n");
        ret = taos_prox_off();
		return ;
    }
	/*change als adc  time to 2.72ms*/
    if ((ret =(i2c_smbus_write_byte_data
		       (taos_datap->client, (TAOS_TRITON_CMD_REG | TAOS_TRITON_ALS_TIME), 0xFF))) < 0) {
            printk(KERN_ERR "TAOS: TAOS_PROX_CALIBRATE failed in set als time \n");
            ret = taos_prox_off();
            return ;
    }
    ...
    if ((ret =(i2c_smbus_write_byte_data
            ...
            printk(KERN_ERR "TAOS: TAOS_PROX_CALIBRATE E failed in set als time\n");
            ret = taos_prox_off();
            return ;
	}
    prox_sum = 0;
    prox_max = 0;
    mdelay(12);//[BUG]-ADD-BEGIN by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
    for (i = 0; i < 20/*10*/; i++) {             //[BUG]-MOD-BEGIN by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
        if ((ret = taos_prox_poll(&prox_cal_info[i])) < 0) {
			...
			mdelay(12);//als time=2.72+wait time 2.72+prox time 2.72,12ms is enough
    }
//[BUG]-DEL-BEGIN by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
//this part change is psensor alg modification ,suggest from supplier
#if 0
   ...
   taos_cfgp->prox_threshold_hi = ((((prox_max - prox_mean) * 200) + 50) / 100) + prox_mean;
    ...
    /*,refer to  mabye some layout design problem, is is easy that proximity value is higher than threhold value in common condition
...
#endif
//[BUG]-DEL-BEGIN by TCTNB.XQJ
//[BUG]-ADD-BEGIN by TCTNB.XQJ,2012/12/26,BUG-367697,for scribe parameter adjust and pass validate test
//this part change is psensor alg modification ,suggest from supplier
    ...
    if (taos_cfgp->prox_threshold_hi< 230){
        taos_cfgp->prox_threshold_hi = 230;
        taos_cfgp->prox_threshold_lo = 150;
    }
    if (taos_cfgp->prox_threshold_hi >900){
        taos_cfgp->prox_threshold_hi = 900;
        taos_cfgp->prox_threshold_lo = 700;
    }
//[BUG]-ADD-END by TCTNB.XQJ
    /*set ALS time*/
    itime = ((((taos_cfgp->als_time * 100 / 50) * 18) / 100) - 1);
    itime = (~itime);
    if ((ret =
...
static int taos_prox_poll(struct taos_prox_info *prxp)
{
	...
	printk("ch4=%d,ch5=%d,prxp->prox_data == %d\n",chdata[4],chdata[5] ,prxp->prox_data);
	prox_history_hi <<= 1;
...
static void taos_prox_poll_timer_func(unsigned long param)
{
	...
	taos_prox_poll_timer_start(200);//2011.12.06  mod by xqj,poll time change from 400 to 200ms
}
...
MODULE_AUTHOR("<Qijun.xu@tcl_mobile.com>");
