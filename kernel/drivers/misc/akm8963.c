...
/* --------|----------------|--------------------|--------------------------- */
/* 12/10/15| Qijun Xu       |                    | Add compass ak8963c driver */
/*---------|----------------|--------------------|--------------------------- */
/******************************************************************************/
...
#define AKM8963_DRDY_TIMEOUT	500
...
struct akm8963_data {
	...
	unsigned char mode;
	int rstn;/*Aeed by NTD.XQJ,2012/08020,for reset IO pass*/
	unsigned char reset_flag;//added by xqj,for reset only reset once at one time
};

static struct akm8963_data *s_akm;
static unsigned char bak_mode;
...
printk( KERN_INFO "RxData: len=%02x, addr=%02x, data=%02x",
		length, i2c->addr, rxData[0]);
#else
int i;
int ret;
     if ((ret = (i2c_smbus_write_byte(i2c,  rxData[0]))) < 0) {
			printk(KERN_ERR "akm8963_i2c_rxdata: i2c_smbus_write_byte() failed \n");
			return (ret);
		}
     for (i = 0; i < length; i++) {
		
		rxData[i] = i2c_smbus_read_byte(i2c);
		//AKM_DATA(&i2c->dev, "RxData: len=%02x, addr=%02x, data=%02x",
		//length, i2c->addr, rxData[i]);
	}
	...
	struct i2c_client *client)
{
	...
	if (buffer[0] != 0x48) {
		dev_err(&client->dev,
			"%s: The device is not AK8963.buffer[0]=%d", __func__,buffer[0]);
		return -ENXIO;
	}
...
static void  akm8963_reset(struct akm8963_data *akm)//reset is only  sepecifal for  8963;
{

	int ret;
	ret = gpio_get_value(s_akm->rstn);
	printk(KERN_INFO "s_akm->rstn=%d,akm8963 GPIO_COMPASS_RESET=%d,reset_flag=%d\n ",s_akm->rstn,ret,s_akm->reset_flag);
    s_akm->reset_flag=1;
    gpio_set_value(s_akm->rstn, 0);
	udelay(120);
	gpio_set_value(s_akm->rstn, 1);
	if(bak_mode==AK8963_MODE_SNG_MEASURE)
    {
        ret = AKECS_SetMode(akm, bak_mode);//avoid because reset,let the now running mode setting appear abort,
	}
		ret = gpio_get_value(s_akm->rstn);
	printk(KERN_INFO "s_akm->rstn=%d,akm8963 GPIO_COMPASS_RESET=%d,reset_flag=%d\n ",s_akm->rstn,ret,s_akm->reset_flag);
	...
	int size)
{
	int err;
	//int i =0;
	/*if (atomic_read(&akm->suspend)) //modified by xqj,avoid in sleep
	{
	   memcpy(rbuf, akm->sense_data, size);//fake data,avoid sleep,old data
	   return 0;
	}*/
...
//
	/*for (i = 0; i < size; i++) {
		printk(KERN_INFO "AKECS_GetData: rbuf[%d] %02x \n", i, rbuf[i]);
	}*/
	...
	int *rbuf)
{
	...
	//ready = (akm->enable_flag & (uint32_t)rbuf[0]);//old board platform 
	ready = akm->enable_flag;


#if 0
	/* Report acc vector information */
	...
	if (ready & MAG_DATA_READY) {
		input_report_abs(s_akm->input, ABS_RX, rbuf[5]);
		input_report_abs(s_akm->input, ABS_RY, rbuf[6]);
		input_report_abs(s_akm->input, ABS_RZ, rbuf[7]);
		input_report_abs(s_akm->input, ABS_RUDDER, rbuf[8]);
		//printk(KERN_INFO "akm8963 ORI_DATA_READY\n ");
	}
	/* Report orientation sensor information */
	if (ready & ORI_DATA_READY) {
		input_report_abs(s_akm->input,ABS_HAT0X, rbuf[9]);
		input_report_abs(s_akm->input, ABS_HAT0Y, rbuf[10]);
		
		input_report_abs(s_akm->input,ABS_HAT1X, rbuf[11]);
		input_report_abs(s_akm->input, ABS_HAT1Y, rbuf[4]);
...
static long
AKECS_ioctl(struct file *file, unsigned int cmd,
			unsigned long arg)
{
	...
	short akm_delay;		/* xqj added ,for alg akmd GET_DELAY */
    ...
    case ECS_IOCTL_GET_DELAY1:
	case ECS_IOCTL_GET_LAYOUT:
   	case ECS_IOCTL_GET_OUTBIT: 
	case ECS_IOCTL_GET_ACCEL:
	case ECS_IOCTL_SET_ACCEL:	
	...
	switch (cmd) {
	case ECS_IOCTL_APP_SET_MFLAG:	      
		akm->enable_flag &= ~(1<<ORI_DATA_FLAG);
	        akm->enable_flag |= ((uint32_t)(flag))<<ORI_DATA_FLAG;
		dev_err(&akm->i2c->dev, "mflag=%d.",akm->enable_flag);
		                akm8963_sysfs_update_active_status(akm);

		break;
	case ECS_IOCTL_APP_SET_MVFLAG:
		akm->enable_flag &= ~(1<<MAG_DATA_FLAG);
	        akm->enable_flag |= ((uint32_t)(flag))<<MAG_DATA_FLAG;
		dev_err(&akm->i2c->dev, "mvflag=%d.",akm->enable_flag);
                akm8963_sysfs_update_active_status(akm);
		break;
	case ECS_IOCTL_READ:
	...
	case ECS_IOCTL_GET_DELAY1:
		...
		akm_delay = akm->delay[1];/*MAG_DATA_FLAG*/
		AKM_DATA(&akm->i2c->dev, "IOCTL_GET_DELAY1 called %d.",akm_delay);
		mutex_unlock(&akm->val_mutex);
		break;
	case ECS_IOCTL_GET_LAYOUT:
		...
		break;
	case ECS_IOCTL_GET_OUTBIT:
		AKM_DATA(&akm->i2c->dev, "IOCTL_GET_OUTBIT outbit=%d",akm->outbit);
		outbit = akm->outbit;
		break;
	...
	case ECS_IOCTL_GET_ACCEL:
		...
		break;
	case ECS_IOCTL_SET_ACCEL:
		//printk(KERN_INFO "IOCTL_SET_ACCEL called.");
		mutex_lock(&akm->accel_mutex);
		if (copy_from_user(&acc_buf, argp, sizeof(acc_buf))) {
			dev_err(&akm->i2c->dev, "copy_from_user failed.");
			return -EFAULT;
		}
		akm->accel_data[0]  =  acc_buf[0] ;
		akm->accel_data[1] =  acc_buf[1];
		akm->accel_data[2] =  acc_buf[2];
		//printk(KERN_INFO "ECS_IOCTL_SET_ACCEL: %d %d %d", acc_buf[0], acc_buf[1], acc_buf[2]);
		mutex_unlock(&akm->accel_mutex);
		break;
	...
	case ECS_IOCTL_GET_CLOSE_STATUS:
		...
		dev_err(&akm->i2c->dev, "status=%d.",status);
	...
	case ECS_IOCTL_GET_DELAY:
		...
		break;
        case ECS_IOCTL_GET_DELAY1:
		if (copy_to_user(argp, &akm_delay, sizeof(akm_delay))) {
			dev_err(&akm->i2c->dev, "copy_to_user failed.");
			return -EFAULT;
		}
		break;
	...
	struct akm8963_data *akm)
{
    ...
    if(s_akm->reset_flag==0 && (en & ORI_DATA_READY)&& (en & MAG_DATA_READY))
	    akm8963_reset(akm);
	dev_dbg(&akm->i2c->dev,
		"Status updated testactive: enable=0x%X, active=%d",
	...
	struct akm8963_data *akm, char const *buf, size_t count, int pos)
{
	...
	//printk(KERN_INFO "akm8963 delay[%d]=%l\n ",pos,val);
	mutex_unlock(&akm->val_mutex);
			...
			size_t size)
{
    ...
    //Alok: dev_get_drvdata is giving NULL for akm
    ...
    //	printk(KERN_INFO "akm8963_bin_accel_write++: %x", akm);
	//printk(KERN_INFO "akm8963_bin_accel_write++: buffer:%s   size=%d", buf, size);
    ...
    //	printk( KERN_INFO "akm8963_bin_accel_write Alok accel:%d,%d,%d\n",
    //		accel_data[0], accel_data[1], accel_data[2]);
    //	printk(KERN_INFO "akm8963_bin_accel_write--");
...
/* [PLATFORM]-Mod-BEGIN by TCTNB.(Qiu Ruifeng), 2012/11/10, PR-350379,
reason [Google CTS]android.permission:android.permission.cts.FileSystemPermissionTest Fail */
	__ATTR(enable_acc, 0771, akm8963_enable_acc_show, akm8963_enable_acc_store),
	__ATTR(enable_mag, 0771, akm8963_enable_mag_show, akm8963_enable_mag_store),
	__ATTR(enable_ori, 0771, akm8963_enable_ori_show, akm8963_enable_ori_store),
	__ATTR(delay_acc,  0771, akm8963_delay_acc_show,  akm8963_delay_acc_store),
	__ATTR(delay_mag,  0771, akm8963_delay_mag_show,  akm8963_delay_mag_store),
	__ATTR(delay_ori,  0771, akm8963_delay_ori_show,  akm8963_delay_ori_store),
/* [PLATFORM]-Mod-END by TCTNB.(Qiu Ruifeng) */
#ifdef AKM8963_DEBUG_IF
...
static struct bin_attribute akm8963_bin_attributes[] = {__BIN_ATTR(accel, 0220, 6, NULL,
...
static irqreturn_t akm8963_irq(int irq, void *handle)
{
	...
	int err;
	int i;
	//printk(KERN_INFO "akm8963_irq: got interrupt\n");
	memset(buffer, 0, sizeof(buffer));
	buffer[0] = AK8963_REG_ST1;
	
	for(i=0;i<100;i++)
	{
	    err = akm8963_i2c_rxdata(akm->i2c, buffer, 1);
	    if (err < 0) {
...
static void akm8963_early_suspend(struct early_suspend *handler)
{
	...
	AKECS_SetMode(s_akm, AK8963_MODE_POWERDOWN);
    ...
    //printk(KERN_INFO "akm8963 testactive akm->reserve_active=%d\n ",s_akm->reserve_active);
...
static void akm8963_late_resume(struct early_suspend *handler)
{
    ...
    if(s_akm->reset_flag==0 && (en & ORI_DATA_READY)&& (en & MAG_DATA_READY))
     ...
     //printk(KERN_INFO "akm8963 testactive akm->active=%d\n ",s_akm->active);
 }
 #endif
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_dbg(&client->dev, "akm8963_probe: start probing.");
    ...
    if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		printk(KERN_ERR "akm: - i2c smbus byte data functions unsupported\n");
		return -EOPNOTSUPP;
	}
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_WORD_DATA)) {
		printk(KERN_ERR "akm: - i2c smbus word data functions unsupported\n");
	}
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_SMBUS_BLOCK_DATA)) {
		printk(KERN_ERR "akm: i2c smbus block data functions unsupported\n");
	}
	...
	/***** Set layout and reset pin information *****/
	pdata = client->dev.platform_data;
	if (pdata) {
		/* Platform data is available. copy its value to local. */
		s_akm->layout = pdata->layout;
        s_akm->outbit = 1;//pdata->outbit;
		s_akm->rstn=pdata->reset;
	} else {
	...
	/***** I2C initialization *****/
	s_akm->i2c = client;
	bak_mode=AK8963_MODE_POWERDOWN;
...
//client->addr=0x0f;
    ...
    s_akm->reset_flag=0;
	s_akm->enable_flag = 0;
	s_akm->delay[0] = 200000000;//default 
	s_akm->delay[1] =30000000;//default 
    s_akm->delay[2] = 200000000;//default;
	/***** IRQ setup *****/
	//pdata->init();
	s_akm->irq =0;// client->irq;
