...
/*[PLATFORM]-Del-BEGIN by TCTNB.YJ, 2013/1/8, add Qualcomm MDP patch to remove kernel error log  */	
//	if (!enable)
//		vsync_cntrl.disabled_clocks = 0;
/*[PLATFORM]-Del-END by TCTNB.YJ, 2013/1/8  */	
	disabled_clocks = vsync_cntrl.disabled_clocks;
	spin_unlock_irqrestore(&mdp_spin_lock, flag);

/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/8, add Qualcomm MDP patch to remove kernel error log  */	
//	if (enable && disabled_clocks) {
	if (enable && disabled_clocks) 
		mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	spin_lock_irqsave(&mdp_spin_lock, flag);
	if (enable && vsync_cntrl.disabled_clocks) {
/*[PLATFORM]-Mod-END by TCTNB.YJ, 2013/1/8  */	
