...
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/14, add sync control integer */	
static int pan_display_status;
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/14, Del begin remove the function of clear FB */	
#if 0
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/14, Del begin remove the function of clear FB */	
	...
	*/
	...
	case 32:
		printk(KERN_EMERG " clear FB start \n");
		memset32_io((void *)fbi->screen_base, 0xFF000000,
							fbi->fix.smem_len);
		printk(KERN_EMERG "clear FB end \n");
		break;
	default:
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/14, add sync between LCD init and LCD backlight  */	
				pan_display_status = 0;
				msleep(25); /*[PLATFORM]-ADD-BEGIN by TCTNB.Liu Jie, 2013/1/17, add sync between LCD init and LCD backlight  */
/*[PLATFORM]-Mod-END by TCTNB.YJ*/
			      ...
			      struct fb_info *info)
{
	...
	up(&msm_fb_pan_sem);
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/14, add sync between LCD init and LCD backlight */	
	if( (unset_bl_level && !bl_updated) || !pan_display_status ){
