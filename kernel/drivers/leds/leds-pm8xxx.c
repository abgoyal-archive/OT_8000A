...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
#include <linux/mfd/pm8xxx/mpp.h>
#define KEYPAD_BACKLIGHT_MPP 166
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/

/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/11/01, Add the torch control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
#include <linux/gpio.h>
#define FLASH_EN_GPIO 2
#define FLASH_MODE_GPIO 3
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
	LED_MAP(PM8XXX_VERSION_8038, 1, 0, 0, 0, 1, 0, 1, 1, 1, 1),
#else
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
static struct pm8xxx_mpp_config_data keyboard_backlight_on_config = {
	.type		= PM8XXX_MPP_TYPE_SINK,
	.level		= PM8XXX_MPP_CS_OUT_15MA,
	.control	= PM8XXX_MPP_CS_CTRL_ENABLE,
};

static struct pm8xxx_mpp_config_data keyboard_backlight_off_config = {
	.type		= PM8XXX_MPP_TYPE_SINK,
	.level		= PM8XXX_MPP_CS_OUT_15MA,
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/10/25, Modify the keyboard backlight control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
    printk("wqf---led_kp_set value=%d\n", value);
    if (value == LED_OFF)
    {
        pm8xxx_mpp_config(KEYPAD_BACKLIGHT_MPP, &keyboard_backlight_off_config);
    }
    else
    {
        pm8xxx_mpp_config(KEYPAD_BACKLIGHT_MPP, &keyboard_backlight_on_config);
    }
#else
	int rc;
	u8 level;
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/11/01, Add the torch control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
    printk("wqf---led_flash_set value=%d\n", value);
    if (value == LED_OFF)
    {
    ...
    {
        gpio_direction_output(FLASH_MODE_GPIO, 0);
        gpio_direction_output(FLASH_EN_GPIO, 1);
    }
#else
	int rc;
	u8 level;
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/11/08, Modify the led indication mode for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
       int i = 0;
       if (led->cdev.brightness != LED_HALF) {
           for (i = 0; i < 2; i++)  //Do two times to make sure the led can light on
           {
#else
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/11/08, Modify the led indication mode for viper*/
       ...
       } else if (led->pwm_duty_cycles != NULL) {
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/11/01, Add the torch control for viper*/
#ifdef CONFIG_MACH_MSM8X30_VIPER
	rc = gpio_request(FLASH_EN_GPIO, "FLASH_EN_GPIO");
	if (rc < 0) {
		pr_err("failed request FLASH_EN_GPIO: %d\n", FLASH_EN_GPIO);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(
			   FLASH_EN_GPIO, 0, GPIO_CFG_OUTPUT,
			   GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("failed FLASH_EN_GPIO tlmm config\n");
		gpio_free(FLASH_EN_GPIO);
		return rc;
	}
	rc = gpio_direction_output(FLASH_EN_GPIO, 0);
	if (rc < 0) {
		pr_err("failed to enable FLASH_EN_GPIO\n");
		gpio_free(FLASH_EN_GPIO);
		return rc;
	}

	rc = gpio_request(FLASH_MODE_GPIO, "FLASH_MODE_GPIO");
	if (rc < 0) {
		pr_err("failed request FLASH_MODE_GPIO: %d\n", FLASH_MODE_GPIO);
		return rc;
	}
	rc = gpio_tlmm_config(GPIO_CFG(
			   FLASH_MODE_GPIO, 0, GPIO_CFG_OUTPUT,
			   GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("failed FLASH_MODE_GPIO tlmm config\n");
		gpio_free(FLASH_MODE_GPIO);
		return rc;
	}
	rc = gpio_direction_output(FLASH_MODE_GPIO, 0);
	if (rc < 0) {
		pr_err("failed to enable FLASH_MODE_GPIO\n");
		gpio_free(FLASH_MODE_GPIO);
		return rc;
	}
#endif
