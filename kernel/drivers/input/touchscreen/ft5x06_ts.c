...
#define FT5X06_REG_PANEL_FACTORY_ID	0xA8
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2013/01/16, add TP Firmware upgrade function*/
...
struct i2c_client *firmware_client;
#endif
/*[PLATFORM]-Mod-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WPL, 2013/02/20, refer to bug399259*/
#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
//get project setting from bootloader
int fts_ctmp_get_project_setting(struct i2c_client *client)
{
	u8 buf[8];
	u8 reg_val[2] = {0};
	...
	*/
	...
	ft5x06_i2c_read(client, buf, 4, buf, 8);
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2013/1/17, make sure CTP already finish startup process*/
                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	err = ft5x06_i2c_read(firmware_client, &reg_addr, 1, &reg_value, 1);
	if (err < 0)
		dev_err(&firmware_client->dev, "version read failed");

	dev_info(&firmware_client->dev, "[FTS] Firmware version = 0x%x\n", reg_value);
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/8/21, Add the wakeup control for scribe5*/
	...
	if (gpio_is_valid(pdata->wakeup_gpio)) {
		...
		if (err) {
			...
			goto free_wakeup_gpio;
		}
		//gpio_set_value_cansleep(data->pdata->wakeup_gpio, 1);
		msleep(FT_WAKEUP_DLY);
	}
#endif
#endif
/*[PLATFORM]-Mod-END by TCTNB.WQF*/
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WPL, 2013/02/20, refer to bug399259*/
#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
	reg_value = fts_ctmp_get_project_setting(client);
	if (reg_value < 0)
		dev_err(&client->dev, "Get Panel factory ID form bootloader failed");

	dev_info(&client->dev, "[FTS] bootloader vendor ID=0x%x\n", reg_value);

	if (reg_value == IS_JUNDA_TP){
		TP_VENDOR = IS_JUNDA_TP;
		printk("vendor: JUNDA TP\n\n");
	}else if (reg_value == IS_MUTTO_TP){
		TP_VENDOR = IS_MUTTO_TP;
...
if(TP_VENDOR == IS_MUTTO_TP){
	if(reg_value < 0x12){
		...
		ret = fts_ctpm_fw_upgrade_with_i_file(client);         //upgrade firmware
	...
	printk("\n\n--by wangpl--, UNKNOWN TP, can't upgrade FW\n\n");
}

#else
     /****************upgrade tp******************/
	if(reg_value < 0x12){
		printk("ft5316 FW upgrade begin...\n");
		ret = fts_ctpm_fw_upgrade_with_i_file(client);         //upgrade firmware
		printk("ft5316 FW upgrade end...\n\n");
		if (ret != 0){
        ...
        if (err < 0)
            dev_err(&client->dev, "version read failed");
        printk("new [FTS] Firmware version = 0x%x\n\n", reg_value);
    }
    /***************upgrade tp end**************/
#endif
#endif
