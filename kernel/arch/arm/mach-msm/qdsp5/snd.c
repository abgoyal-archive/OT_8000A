 ...
 * -----------------------------------------------------------------------------
 * 12/11/12       Qifeng Wu       Move the ext amp control to modem for scribe5
 * 12/09/14       Qifeng Wu       Add the ext amp control for speaker mode for scribe5
 * -----------------------------------------------------------------------------
 */
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/9/14, Add the ext amp control for speaker mode for scribe5*/
//#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
#if 0
#include <linux/gpio.h>

#define GPIO_EXT_AMP_EN 13
...
typedef enum {
  ...
  SND_DEVICE_FM_DIGITAL_STEREO_HEADSET   = SND_DEVICE_DEFAULT+26,
  SND_DEVICE_FM_DIGITAL_SPEAKER_PHONE    = SND_DEVICE_DEFAULT+27,
  SND_DEVICE_FM_DIGITAL_BT_A2DP_HEADSET  = SND_DEVICE_DEFAULT+28,
  SND_DEVICE_FM_RADIO_STEREO_HEADSET     = SND_DEVICE_DEFAULT+29,
  SND_DEVICE_FM_RADIO_SPEAKER_PHONE      = SND_DEVICE_DEFAULT+30, 
  SND_DEVICE_STEREO_HEADSET_AND_SPEAKER  = SND_DEVICE_DEFAULT+31,
  SND_DEVICE_HEADSET_AND_SPEAKER         = SND_DEVICE_DEFAULT+32,
  SND_DEVICE_FM_RADIO_HANDSET            = SND_DEVICE_DEFAULT+33, 
  SND_DEVICE_FM_RADIO_HEADSET            = SND_DEVICE_DEFAULT+34, 
  SND_DEVICE_FM_ANALOG_STEREO_HEADSET       = SND_DEVICE_DEFAULT+35,      /* Low Power Analog FM on Stereo headset FM to pmic */
  SND_DEVICE_FM_ANALOG_STEREO_HEADSET_CODEC = SND_DEVICE_DEFAULT+36,  /* Low Power Analog FM on stereo headset with pmic mixing enabled */
  SND_DEVICE_MAX,

  /* Current device */
  SND_DEVICE_CURRENT                     = (SNDDEV_DUMMY_DATA_UINT32_MAX-1),
...
//#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
		...
		if (dev.device == SND_DEVICE_SPEAKER_PHONE
			|| dev.device == SND_DEVICE_IN_S_SADC_OUT_SPEAKER_PHONE
			|| dev.device == SND_DEVICE_FM_DIGITAL_SPEAKER_PHONE
			|| dev.device == SND_DEVICE_STEREO_HEADSET_AND_SPEAKER)
		{
			if (!is_ext_amp_on)
			{
				gpio_direction_output(GPIO_EXT_AMP_EN, 1);
				is_ext_amp_on = 1;
			}
		} else if (dev.device != SND_DEVICE_CURRENT) {
			if (is_ext_amp_on)
			{
				gpio_direction_output(GPIO_EXT_AMP_EN, 0);
				is_ext_amp_on = 0;
			}
		}
...
//#ifdef CONFIG_MACH_MSM8X25_SCRIBE5
#if 0
	rc = gpio_request(GPIO_EXT_AMP_EN, "gpio_ext_amp_en");
	if (rc < 0) {
		pr_err("failed request GPIO_EXT_AMP: %d\n", GPIO_EXT_AMP_EN);
		return rc;
	}

	rc = gpio_tlmm_config(GPIO_CFG(
			   GPIO_EXT_AMP_EN, 0, GPIO_CFG_OUTPUT,
			   GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc < 0) {
		pr_err("failed GPIO_EXT_AMP_EN tlmm config\n");
		return rc;
	}
	rc = gpio_direction_output(GPIO_EXT_AMP_EN, 0);
	if (rc < 0) {
		pr_err("failed to enable GPIO_EXT_AMP_EN\n");
		gpio_free(GPIO_EXT_AMP_EN);
		return rc;
	}
#endif
/*[PLATFORM]-Add-END by TCTNB.WQF*/
