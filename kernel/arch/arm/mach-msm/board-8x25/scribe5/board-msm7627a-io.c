 ...
 * -----------------------------------------------------------------------------
 * 12/10/31       Qifeng Wu       Add flash led control for scribe5
 * 12/10/08       Qifeng Wu       Change volume up&down keycode for scribe5
 * 12/09/28       Qifeng Wu       Add leds control for scribe5
 * 12/09/14       Qifeng Wu       Add keypad backlight device for scribe5
 * -----------------------------------------------------------------------------
 */
...
// [FEATURE]-ADD-BEGIN by TCTNB.XQJ, 2012/10/15,
// FR-316016, scribe sensor driver dev
#ifdef CONFIG_SENSORS_AK8963 
#include <linux/akm8963.h>   
#define COMPASS_DRDY 18 
#endif
// [FEATURE]-ADD-END by TCTNB.XQJ

// [FEATURE]-ADD-BEGIN by TCTNB.XQJ, 2012/08/20,
// FR-316014, FR-316022,FR-316019, scribe sensor driver dev
...
static struct msm_handset_platform_data hs_platform_data = {
	.hs_name = "7k_handset",
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/11/16, Disable the end key function for scribe5*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/8/21, Modify the gpio define of TP for scribe5*/
#define FT5X06_IRQ_GPIO		48
#define FT5X06_RESET_GPIO	126  //qrd is 26
#define FT5X06_WAKEUP_GPIO	94
/*[PLATFORM]-Mod-END by TCTNB.WQF*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/8/21, Modify the touch key define for scribe5*/
#if 0
    ...
    ":" __stringify(EV_KEY) ":" __stringify(KEY_HOME)   ":220:842:100:44"
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/8/21, Modify the size of TP and add wakeup control for scribe5*/
	.x_max		= 480, //320,
	.y_max		= 800, //480,
	.reset_gpio	= FT5X06_RESET_GPIO,
	.wakeup_gpio = FT5X06_WAKEUP_GPIO,
/*[PLATFORM]-Mod-END by TCTNB.WQF*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/8/21, Add wakeup control for scribe5*/
#if 0
	rc = gpio_tlmm_config(GPIO_CFG(FT5X06_WAKEUP_GPIO, 0,
			GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN,
			GPIO_CFG_8MA), GPIO_CFG_ENABLE);
	if (rc)
		pr_err("%s: gpio_tlmm_config for %d failed\n",
			__func__, FT5X06_WAKEUP_GPIO);
#endif
/*[PLATFORM]-Mod-END by TCTNB.WQF*/
...
#else
static struct led_info leds_info[] = {
        {
	    .name           = "led_green",
	    .flags          = PMIC_GPIO_1,
        },
        {
	    .name           = "led_red",
	    .flags          = PM_MPP__I_SINK__LEVEL_10mA << 16 | PM_MPP_8,
        },
        {
	    .name           = "flash_led",
	    .flags          = 96,
...
static struct led_platform_data leds_pdata = {
	.leds = leds_info,
	.num_leds = 3,
};
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/9/14, Add keypad backlight device for scribe5*/
...
// [FEATURE]-ADD-BEGIN by TCTNB.XQJ, 2012/10/15,
// FR-316016,scribe compass sensor driver dev
#ifdef CONFIG_SENSORS_AK8963
#define GPIO_COMPASS_RESET 34
...
/*[PLATFORM]-Del-BEGIN by TCTNB.WJ, 2012/11/14, Scribe5 not used gpio config to decrease consumption. */
#if 0
...
static struct msm_gpio bma_acc_gpio_cfg_data[] = {
	{
		GPIO_CFG(GPIO_ACC_INT1, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
		"bma_acc_int1"
	},
	{
		GPIO_CFG(GPIO_ACC_INT2, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_6MA),
		"bma_acc_int2"
	},
};
#endif
...
void __init qrd7627a_add_io_devices(void)
{
	...
	/* touchscreen */
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/8/21, Using ft5x06 for scribe5*/
...
//FR-316016,scribe compass sensor driver dev
#ifdef CONFIG_SENSORS_AK8963
	rc = gpio_tlmm_config(GPIO_CFG(GPIO_COMPASS_RESET, 0,
...
/*[PLATFORM]-Del-BEGIN by TCTNB.WQF, 2012/10/08, No need to change the keycode of volume up&down for scribe5*/
#if 0
	/* keypad */
	if (machine_is_msm8625_evt())
