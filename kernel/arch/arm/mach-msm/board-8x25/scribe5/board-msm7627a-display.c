...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/9/5, Change from cmd mode to video mode for LCD*/
...
static int evb_backlight_control(int level, int mode)
{
	...
	int i = 0;
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/9/12, Modify the backlight control for SGM3727*/
#if 1
	int ret = 0;
	int pulse;
	int inc_pulse;
	static int prev_pulse = 0;
	static int first_time_on = 1;
	const int GPIO_LCD_BACKLIGHT_EN = 39;
#else
	int remainder, ret = 0;
#endif
/*[PLATFORM]-Mod-END by TCTNB.WQF*/
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/9/12, Modify the backlight control for SGM3727*/
#if 1
		if (first_time_on) //To make sue the prev_pulse is right when enter this function at the first time
		{
			gpio_set_value(GPIO_LCD_BACKLIGHT_EN, 0);	//turn off the backlight
/*[PLATFORM]-Mod-BEGIN by TCTNB.YJ, 2013/1/10, mod to remove kernel error logs */
			//msleep(5);
			mdelay(5);
...
/*[PLATFORM]-Del-BEGIN by TCTNB.WQF, 2012/9/28, Add leds control for scribe5*/
#if 0
		ret = pmapp_disp_backlight_set_brightness(level);
		 if (ret)
			pr_err("%s: can't set lcd backlight!\n", __func__);
...
static int mipi_NT35510_rotate_panel(void)
{
	...
	if (machine_is_msm8625_evt())
		rotate = 0;  //[PLATFORM]-Mod by TCTNB.WQF, 2012/8/21, No need to rotate for scribe5
...
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/9/6, Modify the gpio config for lcd and backlight*/
#define GPIO_QRD3_LCD_BRDG_RESET_N	129 //qrd is 85
#define GPIO_QRD3_LCD_BACKLIGHT_EN	39  //qrd is 96
#define GPIO_QRD3_LCD_EXT_2V85_EN	35
//#define GPIO_QRD3_LCD_EXT_1V8_EN	40  //No need for scribe5
...
/*[PLATFORM]-Del-BEGIN by TCTNB.WQF, 2012/9/12, No need to control the backlight here for scribe5*/
#if 0
...
/*[PLATFORM]-Add-BEGIN by TCTNB.WQF, 2012/9/12, Turn off the backlight here for scribe5*/
		evb_backlight_control(0, 0);
/*[PLATFORM]-Add-END by TCTNB.WQF*/


/*[PLATFORM]-Mod-BEGIN by TCTNB.WJ, 2012/11/14, Scribe5 decrease the idle consumption. */
#if 1
        gpio_set_value_cansleep(GPIO_QRD3_LCD_BACKLIGHT_EN, 0);
        gpio_set_value_cansleep(GPIO_QRD3_LCD_BRDG_RESET_N, 0);
#else
