...
{
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/9/10, config the detect gpio of sdcard for scribe5*/
...
static unsigned int msm7627a_sdcc_slot_status(struct device *dev)
{
	...
	if (status) {
		...
		if (!status) {
/*[PLATFORM]-Mod-BEGIN by TCTNB.WQF, 2012/9/10, config the detect gpio of sdcard for scribe5*/				if (machine_is_msm7627a_qrd1() ||
