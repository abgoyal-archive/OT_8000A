 ...
 * --------------------------------------------------------------------------------------
 * 12/09/17      Zhang Lu       Modify power, gpio, mask unused sensor driver for scribe5
 * --------------------------------------------------------------------------------------
 * 13/01/17      Su Pengdong    Modify mount_angle,sensor, corrcet the angle for preview
 * --------------------------------------------------------------------------------------
 */
...
#define GPIO_SKU3_CAM_5MP_SHDN_N   120         /* PWDN */
#define GPIO_SKU3_CAM_5MP_CAMIF_RESET   121    /* (board_is(EVT))?123:121 RESET */
#define GPIO_SKU3_CAM_5MP_CAM_DRIVER_PWDN 33  //[PLATFORM]-Mod by TCTNB.ZL,2012/11/05, enable GPIO control power for main camera AF
...
static struct camera_vreg_t msm_cam_vreg[] = {
	...
	{"rfrx1", REG_LDO, 2850000, 2850000, 0},
	{"gp2", REG_LDO, 2800000, 2800000, 0},
};
#if 0
...
static struct msm_camera_sensor_platform_info sensor_board_info_ov7692 = {
	//.mount_angle = 90,
	.mount_angle = 270,//[BUGFIX]-Mod by pengdong.su,384687,13/01/17,mod the mount_angle of ov7692 to 270
	.cam_vreg = msm_cam_vreg,
	.num_vreg = ARRAY_SIZE(msm_cam_vreg),
...
static struct msm_actuator_info msm_act_main_cam_5_info = {
	.board_info     = &msm_act_main_cam_i2c_info,
	.cam_name   = MSM_ACTUATOR_MAIN_CAM_6,//[PLATFORM]-Mod by TCTNB.ZL,2012/10/24, modify configuration for main camera AF
	.bus_id         = MSM_GSBI0_QUP_I2C_BUS_ID,
	.vcm_pwd        = GPIO_SKU3_CAM_5MP_CAM_DRIVER_PWDN,
	.vcm_enable     = 1, //[PLATFORM]-Mod by TCTNB.ZL,2012/11/05, enable GPIO control power for main camera AF
};
...
static struct msm_camera_sensor_flash_src msm_flash_src_ov5647 = {
	...
	._fsrc.ext_driver_src.led_en = 96, //[PLATFORM]-Mod by TCTNB.WQF, 2012/9/5, Modify the gpio for flash led
	._fsrc.ext_driver_src.led_flash_en = 128, //TODO[PLATFORM]-Mod by TCTNB.WQF, 2012/9/5, 
                                                  //scribe5 only use gpio 96 to control the flash led, 128 will be changed later.
};
